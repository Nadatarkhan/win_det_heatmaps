...
class facade(IMDB):
    ...
    def plot(windows_list, imdb_list, save_path):
        ...
        for s_idx in range(len(windows_list)):
          ...
          #NT:Creating dataframe with window points and computing polygon
                name= im[33:45]
                #name= im[37:45]
                df=pd.DataFrame(temp['position'], columns= ['x', 'y'])
                df['img'] = name
                print(df)
                #print(winPred)
                df.to_csv("/content/result.csv", mode="a", header=True)

                #for i in df.iterrows():
                  #coords=i
                  #polygon = Polygon(coords)
                  #window_area= polygon.area
                  #print(coords)
                  
              
                w1  = ((526, 0), (525, 66), (606, 68), (603, 5))
                polygon = Polygon(w1)
                w1_area= polygon.area

                w2  = ((328, 5), (326, 73), (450, 71.6), (450, 1.6))
                polygon = Polygon(w2)
                w2_area= polygon.area

                w3  = ((23, 8.3), (23, 78), (138, 76), (138, 8.3))
                polygon = Polygon(w3)
                w3_area= polygon.area

                w4  = ((175, 8.3), (175, 75), (295, 71), (293, 6.6))
                polygon = Polygon(w4)
                w4_area= polygon.area

                window_area= w1_area+w2_area+w3_area+w4_area
                ...
                #https://stackoverflow.com/questions/47657795/how-to-calculate-area-of-polygon-from-list-of-points-with-python
